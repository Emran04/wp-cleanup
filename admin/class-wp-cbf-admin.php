 ...
 * @package    Wp_Cbf
 * @subpackage Wp_Cbf/admin
 * @author     Emran <emran@emrangist.com>
 */
class Wp_Cbf_Admin {
	...
	**/
	...
	public function validate($input) {
        ...
        if ( !empty($valid['login_background_color']) && !preg_match( '/^#[a-f0-9]{6}$/i', $valid['login_background_color']  ) ) { 
        ...
        if ( !empty($valid['login_button_primary_color']) && !preg_match( '/^#[a-f0-9]{6}$/i', $valid['login_button_primary_color']  ) ) {
        	// if user insert a HEX color with #
            add_settings_error(
